...
io.on('connection', socket => {
  ...
  socket.on('answer:share', ({ code, playerId, slot }, done) => { const room = roomFor(code); if (!room || !shareAnswer(room, playerId, slot)) return done?.({ error: '내 차례에 아직 공개하지 않은 답을 골라 주세요.' }); broadcast(room); done?.({ ok: true }); });
